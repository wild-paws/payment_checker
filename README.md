...
Видео хранятся 3 дня, файлы отчётов — 30 дней. Устаревшие файлы удаляются автоматически перед каждым запуском.
...
Это нормально — запусти упавший тест повторно, профиль постепенно насыщается:
...
добавь на класс теста маркер `clear_session`.
...
Маркер открывает сайт, чистит куки только конкретного домена (не глобально — глобальная очистка
ломает профиль), и при `strategy="full"` дополнительно чистит localStorage, sessionStorage и IndexedDB.
...
После сверки нужные адреса добавляются в `KNOWN_WALLETS` в `.env`.
...
Хороший промт описывает путь пошагово — каждый шаг это клик или ввод, каждый переход — что происходит со страницей
(ререндер DOM или новый URL). Такое описание Codex переводит в page objects один к одному.
...
>
> Происходит ререндер DOM дерева, ссылка возвращается на `https://example-casino.com/`
>
> **2) Главная страница:**
>
> Жму кнопку депозита в шапке: `//a[text()='Deposit']`
>
> Происходит ререндер DOM дерева, URL меняется на `https://example-casino.com/deposit`
>
> **3) Страница депозита:**
>
...
>
> Происходит полная перезагрузка страницы, ссылка меняется на домен провайдера
>
...
> Жму кнопку подтверждения: `//button[@data-test='submit']`
>
> Происходит ререндер DOM дерева, URL меняется на `https://example-casino.com/`
>
> **2) Главная страница:**
>
...
Если тест упал — смотри видео в отчёте (вкладка Attachments → video), пиши Codex что именно не
...
│   └── videos/                     # видео при падении — хранятся 3 дня
├── config/
│   ├── __init__.py
