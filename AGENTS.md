...
**`__init__.py` — единственное место где определяется домен сайта.** Всегда заполняй его так:
...
BASE_URL = "https://example-casino.com"

# Чистый домен для wallet_log — без схемы и www.
SITE = urlparse(BASE_URL).netloc.removeprefix("www.")
```

Для www-доменов всё работает автоматически:
`BASE_URL = "https://www.example-casino.com"` → `SITE = "example-casino.com"`
...
Не определяй `BASE_URL` или `SITE` нигде кроме `__init__.py` — только импортируй.
...
Маркер всегда принимает `BASE_URL` — импортируется из `__init__.py` пакета сайта.
...
```python
# test_site_example.py
from pages.site_example import BASE_URL  # ← домен определён в __init__.py пакета

@pytest.mark.clear_session(BASE_URL, strategy="full")  # если нужна очистка сессии
...
Импортируется из `__init__.py` пакета сайта: `from pages.site_example import SITE`.
