...
pages/site_название/
    ...
    payment_page.py       ← финальная: проверка интеграции и кошелёк
...
внутри iframe (выбор способа оплаты, ввод суммы, адрес кошелька), а URL больше не меняется — это один `PaymentPage`.
Сохраняй `frame_locator` в `self._frame` в `__init__` и используй его для всех действий внутри iframe.

**Паттерн — это способ проверки, не структура пути.** Паттерн 1/2/3 определяет только то, *как* проверяется интеграция
(логотип / API / адрес кошелька). Сам пользовательский путь до этой проверки может быть любым — 2 страницы или 5,
с модалкой или без, с iframe или с редиректом. Главное — отразить реальный путь честно.

**Сокращай путь где возможно.** Если страница или модалка открывается напрямую через URL — открывай через `goto()`
вместо того чтобы искать кнопку и кликать. Это короче, стабильнее и проще читается.
...
## Про паттерны и навигацию

**Паттерн — это способ проверки, а не способ навигации.**
Паттерн 1 / 2 / 3 означает как именно проверяется интеграция: по логотипу, по API ответу или по адресу кошелька.
Путь до этой проверки — отдельная история, он определяется сайтом, а не паттерном.

**Предпочитай прямые ссылки кликам.** Если можно открыть нужный экран напрямую по URL — делай так.
Меньше кликов = меньше точек отказа = стабильнее тест.

Примеры:
- Форма входа по адресу `/login` — `goto("/login")`, не ищи кнопку на главной
- Модалка через параметр `?modal=login` — `goto("/?modal=login")`, не ищи кнопку
- Страница депозита по прямому URL — `goto("/account/cashier/deposit/1076")` после логина, не кликай через меню

Если пользователь в промте описывает переход по прямой ссылке — реализуй именно так.

**Количество page objects определяется путём, а не паттерном.**
Паттерн 3 может быть реализован как через 2 класса (логин → депозит по URL),
так и через 4 (логин → главная → выбор валюты → реквизиты) — зависит от конкретного сайта.
