...
Добавляй маркер на тест или класс если сайт сохраняет авторизацию в браузерном профиле и заходит уже залогиненным.

```python
# По умолчанию — чистит только куки домена
@pytest.mark.clear_session("https://site.com")

# strategy="full" — куки + localStorage + sessionStorage + IndexedDB
...
Если тест упал на капче — запусти только упавший тест повторно:
```bash
